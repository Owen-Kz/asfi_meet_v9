 ...
 * incompatible with ASFI Meet on the client and/or server sides. The main
...
export function parseStandardURIString(str: string) {
    ...
    // whitespace. Anyway, the ASFI Meet app is not known to utilize unencoded
...
/**
 * Parses a specific URI which (supposedly) references a ASFI Meet resource
 * (location).
 *
 * @param {(string|undefined)} uri - The URI to parse which (supposedly)
 * references a ASFI Meet resource (location).
 * @public
 * @returns {{
 *     contextRoot: string,
 *     hash: string,
...
export function parseURIString(uri?: string): any {
    ...
    // XXX While the components/segments of pathname are URI encoded, ASFI Meet
    // on the client and/or server sides still don't support certain characters.
    obj.pathname = obj.pathname.split('/').map((pathPart: any) => _fixPathPart(pathPart))
        .join('/');

    // Add the properties that are specific to a ASFI Meet resource (location)
