...
echo "Releasing ASFI Meet SDK ${SDK_VERSION}"
...
# Now build and publish the ASFI Meet SDK and its dependencies
echo "Building and publishing the ASFI Meet SDK"
pushd ${THIS_DIR}/../
./gradlew clean
...
if [[ $MVN_HTTP == 0 ]]; then
    pushd ${MVN_REPO_PATH}
    git add -A .
    git commit -m "ASFI Meet SDK + dependencies: ${SDK_VERSION}"
    popd
fi
